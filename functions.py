...
def interpolation(N, k, ti, fi,anm_offline):
      
    p_grad = grad(teta_ls, phi_ls, k, N, ti, fi,anm_offline) # grad of the pressure
    u = calc_u(p_grad, ro, k, f[fi])  # local velocity calculation using euler momentum equation with pressure grad
    p = pressure_field(fi, ti, N,anm_offline)  # extrapolated pressure at r 
...
def grad(teta_ls, phi_ls, k, N, ti, fi,anm_offline):
    teta = teta_ls
    phi = phi_ls
    ...
    matrix1 = sph_harm_offline.T
    matrix2 = anm_offline[:, fi,ti][:, np.newaxis]*4*pi*k*spherical_jn_der_offline_ext[fi,:][:, np.newaxis]
    pr_der = np.matmul(matrix1, matrix2).T
#    print("pr_der yeni= ", pr_der)

    # pteta_der
    
    matrix3 = sph_harm_offline_der.T
    matrix4 = anm_offline[:, fi,ti][:, np.newaxis]*4*pi*spherical_jn_offline_ext[fi,:][:, np.newaxis]
    pteta_der = np.matmul(matrix3, matrix4).T
#    print("pteta_der yeni= ", pteta_der)

    
    # pphi_der
    
    matrix5 = sph_harm_offline_m.T
    matrix6 = anm_offline[:, fi,ti][:, np.newaxis]*4*pi*spherical_jn_offline_ext[fi,:][:, np.newaxis]*1j
    pphi_der = np.matmul(matrix5, matrix6).T
#    print("pphi_der yeni= ", pphi_der)

    grad_sum = r_unit * pr_der.T + t_unit * pteta_der.T*(1/rc) + p_unit * pphi_der.T*(1/(rc*np.sin(teta)[:, np.newaxis]))
#    grad_sum = r_unit * pr_der[:, np.newaxis] + t_unit * pteta_der[:, np.newaxis]*(1/rc) + p_unit * pphi_der[:, np.newaxis]*(1/(rc*np.sin(teta)[:, np.newaxis]))

    return(grad_sum)
    
#def a_nm(n, fi, ti, counter,pq_Ymn):
...
def pressure_field(fi, ti, N,anm_offline): 
    """ extrapolated pressure at distance r due to a monochromatic plane wave coming from *** (teta,azimuth) """
    
    matrixa1 = sph_harm_offline.T
    
    
    matrixb1 = anm_offline[:, fi,ti][:, np.newaxis]*4*pi*spherical_jn_offline_ext[fi,:][:, np.newaxis]
    
#    matrixb1 = anm_offline[:, fi,ti]*4*pi*spherical_jn_offline_ext[fi,:]
...
#
#            term_ = anm_offline[counter, fi,ti]*4*np.pi*spherical_jn_offline[fi,n]*sph_harm_offline[counter,:]
#            
#            pr = pr + term_
#            counter = counter + 1
