...
def grad(teta_ls, phi_ls, k, N, ti, fi,anm_offline, rc, **o):
    teta = teta_ls
    phi = phi_ls
...
#    kr = k*rc
    ...
    matrix2 = anm_offline[:, fi,ti][:, np.newaxis]*4*np.pi*k*o['spherical_jn_der_offline_ext'][fi,:][:, np.newaxis]
    pr_der = np.matmul(matrix1, matrix2).T

    # pteta_der
    
    matrix3 = o['sph_harm_offline_der'].T
    matrix4 = anm_offline[:, fi,ti][:, np.newaxis]*4*np.pi*o['spherical_jn_offline_ext'][fi,:][:, np.newaxis]
    ...
    matrix6 = anm_offline[:, fi,ti][:, np.newaxis]*4*np.pi*o['spherical_jn_offline_ext'][fi,:][:, np.newaxis]*1j
...
def calc_u(pressure_grad, k, f, ro):
    w = 2*np.pi*f
...
def pressure_field(fi, ti, N,anm_offline,**o): 
    ...
    matrixb1 = anm_offline[:, fi,ti][:, np.newaxis]*4*np.pi*o['spherical_jn_offline_ext'][fi,:][:, np.newaxis]
...
def interpolation(N, k, f, ti, fi,anm_offline, teta_ls, phi_ls, rc, **o):
      
    p_grad = grad(teta_ls, phi_ls, k, N, ti, fi,anm_offline, rc, **o) # grad of the pressure
    u = calc_u(p_grad, k, f[fi], ro=1)  # local velocity calculation using euler momentum equation with pressure grad
...
def ls_signals(I_n_ls, pressure_ls, L_ra_unit):
